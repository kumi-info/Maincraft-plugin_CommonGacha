...
| `/gacha test <1-6>` | **演出確認用**：指定の星ランク(1〜6)の演出だけ再生（commands 非実行・確率不変・ガチャ名なし） |
...
当選確率に応じて 6 段階で自動的に決まります（確率が低いほど高ランク）。

| ランク | 当選確率 | ⭐ | 色 |
|---|---|---|---|
| UR | 0.5% 以下 | ⭐6 | **RAINBOW（虹色アニメ）** |
| HR | 1% 以下 | ⭐5 | GOLD |
...
- **UR（虹色）**: `color: "RAINBOW"` を指定すると、タイトルが1文字ずつ色相をずらした虹色グラデーションになり、色が流れるアニメーションで表示されます（スロット回転・ランク表示・当選表示・チャットの `%rank%` すべて対応）。当選時の花火も**虹7色×5発**になります。RAINBOW は UR 専用ではなく、任意のランクの `color` に指定できます。
...
rarity-tiers:             # 6段階（rank / max-chance / stars / color）
  - { rank: "UR",  max-chance: 0.5,   stars: 6, color: "RAINBOW" }  # 虹色アニメ
...
| バージョン | 変更点 |
|---|---|
| v1.8.0 | **UR（ウルトラレア・⭐6・当選確率0.5%以下）を最高レアとして追加**。色に `RAINBOW` を新設＝タイトルを1文字ずつ虹色グラデ＋色が流れるアニメで表示（スロット回転・ランク表示・当選表示・チャット `%rank%` 対応）。UR 当選時の花火は**虹7色×5発・STAR型**。`test` コマンドは `<1-6>` に拡張。既存サーバーは `plugins/CommonGacha/config.yml` の `rarity-tiers` 先頭に UR を追記 → `/gacha reload`（jar 差し替えは要再起動）。 |
